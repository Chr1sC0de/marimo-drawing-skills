...
```bash
python diagrams/scripts/check_diagrams_env.py
```

3. Choose node classes by inspecting the installed package when possible:

```bash
python diagrams/scripts/list_diagrams_nodes.py --provider aws --query lambda
python diagrams/scripts/list_diagrams_nodes.py --provider k8s --category compute
```
...
5. Render by running the diagram script or the `diagrams` CLI, then verify that the expected output file exists and is non-empty.
...
- Prefer `svg` for version-controlled diagrams and `png` for documents or chat previews unless the user specifies otherwise.
